...
data class Campo(val linha: Int, val coluna: Int){
    ...
    val objetivo_alcancado: Boolean get() = seguro && aberto || minado && marcado
    val qt_vizinhos: Int get() = vizinhos.filter{it.minado}.size
    val vizinhanca_segura: Boolean
        get() = vizinhos.map{it.seguro}.reduce{resultado, seguro -> resultado && seguro}

    fun add_vizinho(vizinho: Campo) {
        vizinhos.add(vizinho)
    }
    fun on_evento(callback: (Campo, CampoEvento) -> Unit) {
    ...
    fun abrir(){
        if(fechado){
            ...
            if(minado){
                ...
                vizinhos.filter{it.fechado && it.seguro && vizinhanca_segura}.forEach{ it.abrir()}
            }
        }
    }
    fun alterar_marcacao() {
        if (fechado) {
            marcado = !marcado
